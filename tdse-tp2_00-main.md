...
Este archivo es el script de arranque (startup) del microcontrolador.
* **Tabla de Vectores de Interrupción (`g_pfnVectors`)**: Define la estructura de memoria de las interrupciones. Carga la dirección inicial del puntero de pila (`_estack`), la rutina de Reset (`Reset_Handler`) y los manejadores de excepciones e interrupciones de periféricos.
